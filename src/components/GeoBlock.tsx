import { useState, useEffect } from "react";
  ...
  async function detectWithFallbacks(): Promise<{ code: string; name: string } | null> {
    ...
    for (const svc of services) {
      try { const result = await svc(); if (result?.code) return result; } catch {}
    }
    return null;
  }
  ...
  export default function GeoBlock({ children }: GeoBlockProps) {
    ...
    useEffect(() => {
      ...
      detectWithFallbacks().then(result => {
    ...
    if (status === "checking") return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center space-y-4">
          ...
          <p className="text-muted-foreground">{t("geoBlock.checking")}</p>
        </div>
      </div>
    );
          ...
          <CardContent className="p-8 text-center space-y-6">
            ...
            {country && (
              <div className="bg-muted rounded-lg p-3 text-sm">
                ...
                <span className="font-semibold">{country.name} ({country.code})</span>
              </div>
            )}
                ...
                <div className="grid grid-cols-2 gap-1 max-h-48 overflow-y-auto text-left">
                  {ALLOWED_COUNTRIES.map((code) => (
                    <button key={code} onClick={() => handleManualSelect(code)}
                      className="text-xs px-2 py-1 rounded hover:bg-muted text-left transition-colors">
