...
export async function verifySessionToken(token: string): Promise<SessionPayload | null> {
  if (!token) { console.warn("[session] No token provided."); return null; }
  try {
    const secret = new TextEncoder().encode(env.appSecret);
    const { payload } = await jose.jwtVerify(token, secret, { algorithms: [JWT_ALG] });
    const { unionId, userId, clientId } = payload;
    if (!clientId) { console.warn("[session] Missing clientId."); return null; }
    if (!unionId && !userId) { console.warn("[session] Missing unionId or userId."); return null; }
