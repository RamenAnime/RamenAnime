...
echo "Fixing boot.ts: try/catch migrations, no SHOW COLUMNS check..."
...
git add api/boot.ts
git commit -m "fix: try/catch DB migrations, no SHOW COLUMNS" && git push origin main || echo "Push failed"
