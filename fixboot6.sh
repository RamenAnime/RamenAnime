...
echo "Fix: Use PUBLIC endpoint + create users table if missing..."
...
echo ""
echo "IMPORTANT: Set DATABASE_URL to PUBLIC endpoint with /test in Render"
echo ""
echo "mysql://2BxJyeoXt25sRpW.root:UBtvblaYn3ctftiI@gateway01.ap-southeast-1.prod.alicloud.tidbcloud.com:4000/test"
echo ""

git add api/boot.ts
git commit -m "fix: create users table if missing, use public endpoint" && git push origin main || echo "Push failed"
