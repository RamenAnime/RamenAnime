...
echo "Adding auto database migration to boot.ts..."
...
git add api/boot.ts
git commit -m "feat: auto database migration on startup" && git push origin main || echo "Push failed"

echo ""
echo "Now go to Render and click:"
echo "  Manual Deploy > Clear Build Cache & Deploy"
echo ""
echo "When the server starts, it will automatically create:"
echo "  - username column"
echo "  - password_hash column"
echo "  - auth_type column"
echo "  - password_reset_tokens table"
echo ""
echo "Then registration will work immediately."
