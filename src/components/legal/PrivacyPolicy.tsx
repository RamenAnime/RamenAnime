...
export default function PrivacyPolicy() {
  ...
  const f = framework;
  const bundle = usePrivacySections();
  ...
  const defaultOpen = bundle.sections[0]?.id ?? "privacy-1";
      ...
      <div className="prose dark:prose-invert max-w-none">
        ...
        <p className="text-sm text-muted-foreground mb-8">
          ...
          {t("legal.applicableLaw", {
            law: f?.privacyLaw ?? t("legal.defaults.globalPrivacyLaw"),
          })}{" "}
          |{" "}
          {t("legal.yourRegion", {
        ...
        {bundle.quickSummaryTitle ? (
          <div className="bg-muted/50 p-4 rounded-lg mb-8 text-sm">
            <p className="font-medium mb-2">{bundle.quickSummaryTitle}</p>
            <p>{interpolateLegal(bundle.quickSummaryBody, vars)}</p>
          </div>
        ) : null}
