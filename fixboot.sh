...
echo "Fixing boot.ts: non-blocking migrations + use Drizzle connection..."
...
git add api/boot.ts
git commit -m "fix: non-blocking DB migrations using Drizzle connection" && git push origin main || echo "Push failed"

echo ""
echo "Now deploy on Render:"
echo "  Manual Deploy > Clear Build Cache & Deploy"
echo ""
echo "The server will start immediately and run DB migrations 2 seconds later."
echo "If DB connection still fails, check that DATABASE_URL is set in Render env vars."
