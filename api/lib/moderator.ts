// Auto-Moderator Engine
// Scans content for TOS violations, spam, and abusive behavior
// Logs all actions to moderation_logs table
...
export function scanContent(content: string, username?: string): ModerationResult {
  ...
  for (const word of BLOCKED_WORDS) {
    if (lower.includes(word)) {
      return { action: "remove", rule: "PROFANITY_FILTER", reason: `Content contains prohibited language: "${word}"`, confidence: 0.95 };
    }
  }
  for (const pattern of SPAM_PATTERNS) {
    if (pattern.test(content)) {
      return { action: "remove", rule: "SPAM_DETECTION", reason: `Content matches spam pattern: ${pattern.source}`, confidence: 0.9 };
    }
  }
  if (hasExcessiveRepetition(content)) {
    return { action: "flag", rule: "REPETITIVE_CONTENT", reason: "Content contains excessive character repetition (likely spam)", confidence: 0.85 };
  }
  if (hasExcessiveCaps(content)) {
    return { action: "flag", rule: "EXCESSIVE_CAPS", reason: "Content is predominantly ALL CAPS", confidence: 0.7 };
  }
  const linkMatches = content.match(/https?:\/\//g);
  if (linkMatches && linkMatches.length > 3) {
    return { action: "flag", rule: "EXCESSIVE_LINKS", reason: `Content contains ${linkMatches.length} external links`, confidence: 0.75 };
  }
