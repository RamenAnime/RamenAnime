...
import { useCurrency, ALL_CURRENCIES } from "@/hooks/useCurrency";

// Products with USD base prices for currency conversion
const allProducts = [
...
export default function Shop() {
  ...
  const { currencyCode, setCurrency, format, detectedCountry } = useCurrency();
      ...
      <div className="container px-4 md:px-6">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mb-10">
          ...
          <div className="flex items-center gap-2">
            <span className="text-xs text-muted-foreground">{detectedCountry ? `Detected: ${detectedCountry}` : "Currency:"}</span>
            <Select value={currencyCode} onValueChange={setCurrency}>
              <SelectTrigger className="w-[140px] text-xs bg-muted/50 border-border/50">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
