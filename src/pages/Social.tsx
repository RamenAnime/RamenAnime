      ...
      <div className="container px-4 md:px-6 max-w-7xl mx-auto">
        <div className="flex flex-col lg:flex-row gap-6">
              ...
              {filteredPosts.map((post) => (
                <Card key={post.id} className={`border-border/50 hover:border-primary/30 transition-all duration-200 ${post.isPinned ? "border-l-2 border-l-primary" : ""}`}><CardContent className="p-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap mb-1"><Link to={`/profile/${post.authorId}`} className="text-sm font-medium text-foreground hover:text-primary transition-colors">{post.author?.name ?? post.author?.username ?? t("common.anonymous")}</Link>{post.author?.rank && (<Badge variant="outline" className={`text-[10px] px-1.5 py-0 ${getRankColor(post.author.rank.color)}`}>{post.author.rank.name}</Badge>)}<span className="text-xs text-muted-foreground">·</span><span className="text-xs text-muted-foreground flex items-center gap-1"><Clock className="h-3 w-3" />{new Date(post.createdAt).toLocaleDateString()}</span>{post.isPinned && (<Badge variant="outline" className="text-xs border-primary/30 text-primary"><Pin className="h-3 w-3 mr-1" />{t("common.pinned")}</Badge>)}{post.isLocked && (<Badge variant="outline" className="text-xs border-orange-500/30 text-orange-400"><Lock className="h-3 w-3 mr-1" />{t("common.locked")}</Badge>)}<Badge variant="outline" className="text-xs border-border/50 text-muted-foreground">{forumCategoryLabel(post.category)}</Badge></div>
          ...
          <div className="w-full lg:w-72 shrink-0 space-y-4">
            ...
            <Card className="border-border/50"><CardContent className="p-4"><h3 className="font-semibold text-foreground flex items-center gap-2 mb-3"><Users className="h-4 w-4 text-primary" />{t("forum.forumStats")}</h3><div className="space-y-1.5 text-sm">{subforumStats?.map((s) => (<div key={s.category} className="flex justify-between text-muted-foreground"><span>{forumCategoryLabel(s.category)}</span><span>{t("forum.threadsCount", { count: s.postCount })}</span></div>))}</div></CardContent></Card>
          </div>
        </div>
      </div>
    </div>
