...
echo "Fixing boot.ts: raw mysql2 with SSL for TiDB Cloud..."
...
git add api/boot.ts
git commit -m "fix: raw mysql2 with SSL for TiDB Cloud DDL migrations" && git push origin main || echo "Push failed"

echo ""
echo "Now deploy on Render:"
echo "  Manual Deploy > Clear Build Cache & Deploy"
echo ""
echo "If this still fails, the guaranteed fix is:"
echo "  1. Go to https://tidbcloud.com"
...
ALTER TABLE users ADD COLUMN auth_type ENUM('oauth','local') DEFAULT 'oauth' NOT NULL;
